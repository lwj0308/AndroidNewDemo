...
//singleton,一个类，只允许生成一个实例：单例模式
//Database实例化，耗资源，不要出现多个，应用中多次呼叫也是返回同一个实例
...
//                    .fallbackToDestructiveMigration()//把现有的数据清空，创建新的结构
    ...
    public abstract WordDao getWordDao();    //若有多个Entity，则要写多个Dao

    //必须常量
    static final Migration MIGRATION_2_3 = new Migration(2, 3) {
        @Override
        public void migrate(@NonNull SupportSQLiteDatabase database) {
            database.execSQL("ALTER TABLE word ADD COLUMN bar_data INTEGER NOT NULL DEFAULT 1");//在数据库中boolean用INTEGER
        }
    };
